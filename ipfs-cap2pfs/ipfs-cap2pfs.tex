...
Industry has gotten away with using HTTP this long because moving small files around is relatively cheap, even for small organizations with lots of traffic. But we are entering a new era of data distribution with new challenges:
\begin{enumerate*}[label=(\alph*)]
    \item hosting and distributing petabyte datasets,
    \item computing on large data across organizations,
    \item high-volume high-definition on-demand or real-time media streams,
    \item versioning and linking of massive datasets,
    \item preventing accidental disappearance of important files,
\end{enumerate*}
and more. Many of these can be boiled down to ``lots of data, accessible everywhere.'' Pressed by critical features and bandwidth concerns, we have already given up HTTP for different data distribution protocols. The next step is making them part of the Web itself.
...
SFS~\cite{mazieres98, mazieres00} proposed compelling implementations of both
\begin{enumerate*}[label=(\alph*)]
    \item distributed trust chains, and
    \item egalitarian shared global namespaces
\end{enumerate*}. SFS introduced a technique for building \textit{Self-Certified Filesystems}: addressing remote filesystems using the following scheme
...
\begin{enumerate*}[label=(\alph*)]
    \item choose the best function for the use case (e.g. stronger security vs faster performance), and
    \item evolve as function choices change
\end{enumerate*}. Self-describing values allow using different parameter choices compatibly.
...
\begin{enumerate*}[label=(\alph*)]
    \item provides resistance to attackers who would create lots of new nodes
(sybill attacks),
    \item protects previously successful trade relationships, even if one of the
nodes is temporarily unable to provide value, and
    \item eventually chokes relationships that have deteriorated until they
improve
\end{enumerate*}.
...
\texttt{want\_list} to all connected peers. This is done
\begin{enumerate*}[label=(\alph*)]
    \item upon opening the connection,
    \item after a randomized periodic timeout,
    \item after a change in the \texttt{want\_list} and
    \item after receiving a new block
\end{enumerate*}.
...
The IPFS Merkle DAG is an extremely flexible way to store data. The only requirements are that object references be
\begin{enumerate*}[label=(\alph*)]
    \item content addressed, and
    \item encoded in the format above
\end{enumerate*}. IPFS grants applications complete control over the data field; applications can use any custom data format they chose, which IPFS may not understand. The separate in-object link table allows IPFS to:
...
\begin{enumerate*}[label=(\alph*)]
    ...
    \item Linked Data triple stores
    \item linked document publishing systems
    \item linked communications platforms
    \item cryptocurrency blockchains.
\end{enumerate*}
...
I hoped to use the Git object formats exactly, but had to depart to introduce certain features useful in a distributed filesystem, namely
\begin{enumerate*}[label=(\alph*)]
    \item fast size lookups (aggregate byte sizes have been added to objects),
    \item large file deduplication (adding a \texttt{list} object), and
    \item embedding of \texttt{commits} into \texttt{trees}
\end{enumerate*}. However, IPFS File objects are close enough to Git that conversion between the two is possible. Also, a set of Git objects can be introduced to convert without losing any information (Unix file permissions, etc).
...
The full power of the Git version control tools is available to IPFS users. The object model is compatible, though not the same. It is possible to
\begin{enumerate*}[label=(\alph*)]
    \item build a version of the Git tools modified to use the IPFS object graph,
    \item build a mounted FUSE filesystem that mounts an IPFS \texttt{tree} as a Git repo, translating Git filesystem read/writes to the IPFS formats
\end{enumerate*}.
...
\begin{enumerate}[label=(\alph*)]
  \item Use Rabin Fingerprints \cite{RabinFingerprints} as in LBFS \cite{LBFS} to pick suitable block boundaries.
  \item Use the rsync \cite{rsync} rolling-checksum algorithm, to detect blocks that have changed between versions.
  \item Allow users to specify block-splitting functions highly tuned for specific files.
\end{enumerate}
...
It is worth stating why -- if mutable data is necessary in the end -- we worked hard to build up an \textit{immutable} Merkle DAG. Consider the properties of IPFS that fall out of the Merkle DAG: objects can be
\begin{enumerate*}[label=(\alph*)]
    \item retrieved via their hash,
    \item integrity checked,
    \item linked to others, and
    \item cached indefinitely
\end{enumerate*}. In a sense:
...
\begin{enumerate}[label=(\alph*)]
  \item an IPFS library to import in your own applications.
  \item commandline tools to manipulate objects directly.
  \item mounted file systems, using FUSE \cite{FUSE} or as kernel modules.
\end{enumerate}
